        ...
        <div className="space-y-6 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-white rounded-2xl p-6 shadow-sm">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            ...
                            <QuickAction
                                ...
                                icon={<Plus size={20} />}
                                color="#10B981"
                                href="/jadwal/kalender"
                            />
                            <QuickAction
                                label="Perubahan Jadwal"
                                icon={<RefreshCw size={20} />}
                                color="#F59E0B"
                                href="/jadwal/perubahan"
                            />
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl p-6 shadow-sm">
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <div className="flex items-center gap-2">
                                <button className="px-3 py-1 text-sm bg-pink-100 text-pink-600 rounded-lg font-medium">
                                    Kampus 1
                                </button>
                        ...
                        <div className="overflow-x-auto">
                            <div className="min-w-[600px]">
                                ...
                                <div className="grid grid-cols-7 gap-2 mb-4">
                                    <div className="text-xs font-medium text-gray-400 text-center">Jam</div>
                                    {['XII PPLG 1', 'XII PPLG 2', 'XI TMS 1', 'XI TMS 2', 'X PPLG 1', 'X TKJ 1'].map((cls) => (
                                        <div key={cls} className="text-xs font-medium text-gray-600 text-center">{cls}</div>
                                    ))}
                                </div>
                        ...
                        <Link
                            href="/jadwal/jadwal-kelas"
                            className="mt-4 flex items-center justify-center gap-2 py-2 text-sm text-pink-600 hover:text-pink-700 hover:bg-pink-50 rounded-lg transition-colors"
                        >
                    ...
                    <div className="bg-white rounded-2xl p-6 shadow-sm">
                        <div className="flex items-center justify-between mb-4">
                            <h2 className="text-lg font-semibold text-gray-800">Jadwal Bentrok</h2>
                        ...
                        <div className="space-y-3">
                            <ConflictCard
                                type="teacher"
                                description="Pak Budi dijadwalkan di 2 kelas berbeda"
                                slot1={{ class: 'XII PPLG 1', time: '10:00 - 11:30' }}
                                slot2={{ class: 'X TKJ 2', time: '10:00 - 11:30' }}
                            />
                            <ConflictCard
                                type="room"
                                description="Lab Komputer 1 digunakan 2 kelas"
                                slot1={{ class: 'XI TMS 1', time: '13:00 - 14:30' }}
                                slot2={{ class: 'XI PPLG 2', time: '13:00 - 14:30' }}
                            />
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white rounded-2xl p-6 shadow-sm">
                        <div className="flex items-center justify-between mb-4">
                            <h2 className="text-lg font-semibold text-gray-800">Event Mendatang</h2>
                            <Link
                                href="/jadwal/kalender"
                    ...
                    <div className="bg-white rounded-2xl p-6 shadow-sm">
                        <div className="flex items-center justify-between mb-4">
                            <h2 className="text-lg font-semibold text-gray-800">Request Perubahan</h2>
                    ...
                    {/* Teacher Workload */}
                    <div className="bg-white rounded-2xl p-6 shadow-sm">
                        <h2 className="text-lg font-semibold text-gray-800 mb-4">Beban Mengajar</h2>
...
function ScheduleRow({
    ...
    time: string;
    slots: { subject: string; teacher: string; highlight?: boolean }[];
}) {
    return (
        <div className="grid grid-cols-7 gap-2">
            ...
            {slots.map((slot, index) => (
                <div
                    key={index}
                    className={`p-2 rounded-lg text-center ${slot.highlight
                            ? 'bg-pink-100 border border-pink-200'
                            : 'bg-gray-50 hover:bg-gray-100'
                        } transition-colors`}
                >
                    <p className={`text-xs font-medium ${slot.highlight ? 'text-pink-700' : 'text-gray-700'}`}>
                        {slot.subject}
                    </p>
                    <p className="text-[10px] text-gray-400 truncate">{slot.teacher}</p>
                </div>
            ))}
        </div>
    );
...
function ConflictCard({
    ...
    type: 'teacher' | 'room';
    description: string;
    slot1: { class: string; time: string };
    slot2: { class: string; time: string };
}) {
    return (
        <div className="p-4 rounded-xl bg-red-50 border border-red-100">
            ...
            <div className="flex gap-4 text-xs text-red-600">
                <div className="flex items-center gap-1">
                    <MapPin size={12} />
                    <span>{slot1.class} ({slot1.time})</span>
                </div>
                <div className="flex items-center gap-1">
                    <MapPin size={12} />
                    <span>{slot2.class} ({slot2.time})</span>
                </div>
            </div>
...
function EventCard({
    ...
    return (
        <div className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 transition-colors">
            ...
            <span className={`text-xs px-2 py-0.5 rounded-full ${typeStyles[type]}`}>
                {typeLabels[type]}
            </span>
        </div>
    );
    ...
    return (
        <div className="p-3 rounded-xl bg-amber-50 border border-amber-100">
            <div className="flex items-start justify-between mb-1">
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full ${type === 'tukar' ? 'bg-blue-100 text-blue-600' : 'bg-orange-100 text-orange-600'
                    }`}>
...
function WorkloadItem({
    ...
    return (
        <div className={`p-3 rounded-xl ${overload ? 'bg-red-50 border border-red-100' : 'bg-gray-50'}`}>
            <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-medium text-gray-800">{teacher}</span>
                <span className={`text-xs ${overload ? 'text-red-600' : 'text-gray-500'}`}>
                    {hours}/{maxHours} jam
                </span>
            </div>
            <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                <div
                    className={`h-full rounded-full ${overload ? 'bg-red-500' : percentage > 90 ? 'bg-amber-500' : 'bg-green-500'
                        }`}
