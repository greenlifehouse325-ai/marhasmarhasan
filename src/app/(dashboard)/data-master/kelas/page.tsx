    ...
    return (
        <div className="space-y-4 sm:space-y-6 animate-fadeIn">
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <Link href="/data-master/kelas/create" className="flex items-center justify-center gap-2 px-4 py-2.5 bg-[var(--brand-primary)] rounded-xl text-white text-sm font-medium w-full sm:w-auto">
                    <Plus size={16} />
                    Tambah Kelas
                </Link>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                <div className="bg-[var(--bg-card)] border border-[var(--border-light)] rounded-xl p-3 sm:p-4">
                    ...
                    <p className="text-lg sm:text-2xl font-bold text-[var(--text-primary)]">{MOCK_CLASSES.length}</p>
                    <p className="text-xs text-[var(--text-muted)]">Total Kelas</p>
                </div>
                <div className="bg-[var(--bg-card)] border border-[var(--border-light)] rounded-xl p-3 sm:p-4">
                    <Users size={18} className="text-green-500 mb-1 sm:mb-2" />
                    <p className="text-lg sm:text-2xl font-bold text-[var(--text-primary)]">{MOCK_CLASSES.reduce((s, c) => s + c.siswa, 0)}</p>
                ...
                <div className="flex gap-2 overflow-x-auto pb-1 sm:pb-0">
                    {['all', 'X', 'XI', 'XII'].map(tingkat => (
                        <button key={tingkat} onClick={() => setTingkatFilter(tingkat)}
                            className={`px-3 py-2 rounded-xl text-xs sm:text-sm font-medium whitespace-nowrap ${tingkatFilter === tingkat ? 'bg-[var(--brand-primary)] text-white' : 'bg-[var(--bg-hover)] text-[var(--text-secondary)]'}`}>
            ...
            <div className="hidden lg:block bg-[var(--bg-card)] border border-[var(--border-light)] rounded-xl overflow-hidden">
                <table className="w-full">
                    ...
                    <tbody className="divide-y divide-[var(--border-light)]">
                        {MOCK_CLASSES.map(kelas => (
                            <tr key={kelas.id} className="hover:bg-[var(--bg-hover)]">
                                ...
                                <td className="px-4 py-3 text-sm">{kelas.siswa}</td>
